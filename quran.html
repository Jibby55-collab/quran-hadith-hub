/* Qur'an Player — MP3Quran-first with Vercel proxy
   - Loads ALL reciters from /api/mp3quran (proxy for mp3quran.net)
   - Each reciter has one or more "moshaf" (style) with a base `server` and `surah_list`
   - Audio URL = `${server}${NNN}.mp3` (NNN = 001..114)
   - If API fails, we show a minimal fallback set with known-good servers.
*/
...
async function init(){
  ...
  if (!data || !data.reciters) data = await fetchJson(API_PROXY_AR);
...
/* Fallback reciters (if API/proxy fails) — known-good servers */
